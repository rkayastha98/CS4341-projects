...
class ExpectimaxCharacter(CharacterEntity):
    ...
    def do(self, wrld):
        ...
        value, dest_cell = self.expectimax_stupid_monster_no_bombs(wrld, (self.x, self.y), monster_loc, None, True, 0)
        dx, dy = dest_cell
    ...
    def heuristic_stupid_monster_no_bombs(self, world, bman, monster, action):
        v = 0
    ...
    def expectimax_stupid_monster_no_bombs(self, world, bman, monster, action, max_node, depth):
        value = 0
        taction = 0
        # Terminal Node
        if depth >= MAX_DEPTH:
            value, taction = self.heuristic_stupid_monster_no_bombs(world, bman, monster, action)
        # Max node
        elif max_node:
            children = utility.get_neighbors(world, bman)
            children.append(bman)
            successors = [self.expectimax_stupid_monster_no_bombs(world, child, monster, (child[0] - bman[0], child[1] - bman[1]), False, depth + 1) for child in children]
        ...
        # Chance node
            ...
            successors = [self.expectimax_stupid_monster_no_bombs(world, bman, child, (child[0] - monster[0], child[1] - monster[1]), True, depth + 1)[0] for child in children]
            # idx = max(range(len(successors)), key=lambda i: successors[i][0])
